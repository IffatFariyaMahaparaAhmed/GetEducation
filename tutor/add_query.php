...
if (isset($_POST['update_question'])) {
    ...
    foreach ($_POST['qsn'] as $q_id => $qsn_status) {
        //print_r($_POST['qsn_id']);
        $quiz_qsn_id = $_POST['quiz_qsn_id'] [$q_id];
        ...
        $updateQuery = "UPDATE `quiz_qsn` SET `qsn` = '$qsn', `option_one` = '$option_one', `option_tow` = '$option_tow', `option_three` = '$option_three', `option_four` = '$option_four', `ans` = '$ans' WHERE `quiz_qsn_id` = '$quiz_qsn_id'";

        $result_qsn = mysqli_query($connect, $updateQuery);

        if ($result_qsn) {
            $_SESSION['success'] = 'QuiZ Question Edited Successful';
            echo "<script>document.location.href='view_qsn.php?qsn=$qsn_id'</script>";
        } else {
            $_SESSION['error'] = 'QuiZ Question Added Failed';

            echo "<script>document.location.href='edit-question.php'</script>";
        }
    }
}
